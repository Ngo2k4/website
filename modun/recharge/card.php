...
if ($_REQUEST) {
    ...
    if ($type === 'recharge-card') {
        ...
        if ($result->status == 99) {
            ...
            mysqli_query($conn, "INSERT INTO table_card(card_username, card_request_id, card_type, card_amount, card_serial, card_pin, card_status, card_time) 
            VALUES ('$user_username', '" . $result->request_id . "', '$card_type', '$card_amount', '$card_serial', '$card_pin', '" . $result->message . "', '$card_time')");
            die(json_encode($return));
        } else if ($result->status == 1) {
            $return['error'] = 1;
            $return['msg']   = 'Nạp thẻ thành công mệnh giá ' . $card_amount . ' VNĐ.';
            mysqli_query($conn, "INSERT INTO table_card(card_username, card_request_id, card_type, card_amount, card_serial, card_pin, card_status, card_time) 
            VALUES ('$user_username', '" . $result->request_id . "', '$card_type', '$card_amount', '$card_serial', '$card_pin', '" . $result->message . "', '$card_time')");
            die(json_encode($return));
            ...
            $return['error'] = 1;
            $return['msg']   = 'Thẻ nạp không đúng định dạng';
            die(json_encode($return));
        }
