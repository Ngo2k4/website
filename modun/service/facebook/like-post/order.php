...
if ($_REQUEST) {
    ...
    if ($type === 'buff-like-post-facebook') {
        ...
        $buff_reaction = htmlspecialchars(addslashes($_POST['buff_reaction']));
        $buff_speed    = htmlspecialchars(addslashes($_POST['buff_speed']));
        ...
        $urlCreateService        = 'https://automxh.vn/api/service/facebook/like-post-speed/order';
        $dataPostCreate          = array(
            'idpost' => $buff_fb_id,
            'server_order' => $buff_server,
            'reaction' => $buff_reaction,
            'speed' => $buff_speed,
            'amount' => $buff_amount,
            'note' => $buff_note
        );
        ...
        if ($user_point < $buff_payment) {
            ...
            $return['error'] = 1;
            $return['msg']   = 'Trường số lượng phải tối thiểu là ' . number_format($check_service['service_min_amount']) . ' Like.';
            die(json_encode($return));
        } else if ($buff_amount > $check_service['service_max_amount']) {
            $return['error'] = 1;
            $return['msg']   = 'Trường số lượng tối đa là ' . number_format($check_service['service_max_amount']) . ' Like.';
            die(json_encode($return));
            ...
            mysqli_query($conn, "INSERT INTO table_buff(buff_username, buff_slug, buff_code, buff_fb_id, buff_server, buff_reaction, buff_speed, buff_amount, buff_note, buff_present, buff_payment, buff_status, buff_time) 
            VALUES ('$user_username','$type', '".$resultCreateDecode->code_order."', '$buff_fb_id', '$buff_server', '$buff_reaction', '$buff_speed', '$buff_amount', '$buff_note', '$buff_present', '$buff_payment', '$buff_status', '$buff_time')");
