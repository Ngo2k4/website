...
if ($_REQUEST) {
    ...
    $type   = $_REQUEST['type'];
    if ($type === 'buff-eye-live-facebook') {
        ...
        $buff_minutes  = htmlspecialchars(addslashes($_POST['buff_minutes']));
        ...
        $buff_payment            = $check_service['service_price'] * $buff_amount * $buff_minutes;
        $history_buy_point_final = $user_point - $buff_payment;
        $service_title           = $check_service['service_title'];
        $urlCreateService        = 'https://automxh.vn/api/service/facebook/eye-live/order';
        $dataPostCreate          = array(
            'idpost' => $buff_fb_id,
            'server_order' => $buff_server,
            'amount' => $buff_amount,
            'minutes' => $buff_minutes,
            'note' => $buff_note
        );
        ...
        if ($user_point < $buff_payment) {
            ...
            mysqli_query($conn, "INSERT INTO table_buff(buff_username, buff_slug, buff_code, buff_fb_id, buff_server, buff_minutes, buff_amount, buff_note, buff_present, buff_payment, buff_status, buff_time) 
            VALUES ('$user_username','$type', '".$resultCreateDecode->code_order."', '$buff_fb_id', '$buff_server', $buff_minutes, '$buff_amount', '$buff_note', '$buff_present', '$buff_payment', '$buff_status', '$buff_time')");
